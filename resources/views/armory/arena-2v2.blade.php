...
@section('title', __('armory.arena_2v2_page_title'))

@section('content')
<div class="armory-main">
    <div class="armory-container">
        <h1 class="armory-title">{{ __('armory.arena_2v2_title') }}</h1>

        @include('armory.partials.navbar')

        @if (count($teams) == 0)
            <div class="no-data">{{ __('armory.arena_2v2_no_teams') }}</div>
        @else
            <div class="table-container">
                <table class="wow-table">
                    <thead>
                        <tr>
                            <th>{{ __('armory.arena_2v2_rank') }}</th>
                            <th>{{ __('armory.arena_2v2_name') }}</th>
                            <th>{{ __('armory.arena_2v2_faction') }}</th>
                            <th>{{ __('armory.arena_2v2_wins') }}</th>
                            <th>{{ __('armory.arena_2v2_losses') }}</th>
                            <th>{{ __('armory.arena_2v2_winrate') }}</th>
                            <th>{{ __('armory.arena_2v2_rating') }}</th>
                        </tr>
                    </thead>
                    <tbody>
                        @php
